...
import { Code, Database, Globe, Server, Terminal, BarChart3, GitBranch, Languages, Grid } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
          ...
          <div className={cn(
            ...
            <div className="mt-8">
              <Link 
                to="/photography" 
                className="flex items-center justify-center gap-2 w-full px-6 py-3 rounded-md border border-border bg-secondary/50 hover:bg-secondary transition-all relative overflow-hidden group"
              >
                <div className="absolute inset-0 grid grid-cols-12 grid-rows-8 opacity-20 group-hover:opacity-30 transition-opacity">
                  {Array.from({ length: 96 }).map((_, i) => (
                    <div key={i} className="border-[0.5px] border-primary/40"></div>
                  ))}
                </div>
                <Grid className="w-5 h-5 mr-1" />
                <span className="relative z-10 font-medium">View Photography</span>
              </Link>
            </div>
          </div>
        </div>
